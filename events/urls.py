...
urlpatterns = patterns('events.views',
    ...
    url(r'^month/(?P<year>\d+)/(?P<month>\d+)/$', EventMonthArchiveView.as_view(allow_future=True), name='events_month'),
    url(r'^week/(?P<year>\d+)/(?P<week>\d+)/$', EventWeekArchiveView.as_view(allow_future=True), name='events_week'),
